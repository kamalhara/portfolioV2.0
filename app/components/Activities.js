    ...
    >
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-12 lg:gap-12">
        <div className="lg:col-span-4">
          ...
          <h2
            id="activity-title"
            className="mt-4 max-w-[12ch] text-4xl font-medium leading-snug tracking-tight text-ink-text"
          >
        ...
        <div className="min-w-0 lg:col-span-8">
          ...
          <div className="overflow-x-auto border-b-[1.5px] border-paper-border py-8 text-ink-muted transition-colors duration-300 hover:border-accent-orange/50">
            <GitHubCalendar
              username="kamalhara"
              colorScheme="light"
              theme={{
      ...
      <div className="mt-20 grid gap-10 lg:grid-cols-12 lg:gap-12">
        <div className="lg:col-span-4">
          ...
          <h2 className="mt-4 text-4xl font-medium tracking-tight text-ink-text">
            Toolbox
          </h2>
          <p className="mt-4 max-w-[34ch] font-serif text-[15px] italic leading-relaxed text-ink-muted">
        ...
        <ol className="border-t-[1.5px] border-paper-border lg:col-span-8">
          {skillCategories.map((category, index) => {
            ...
            return (
              <li
                key={category.title}
                className="group/category grid grid-cols-[2rem_minmax(0,1fr)] gap-x-4 border-b-[1.5px] border-paper-border py-6 sm:grid-cols-[2.5rem_8rem_minmax(0,1fr)] sm:gap-x-6"
              >
                <span
                  aria-hidden="true"
                ...
                <ul className="col-start-2 mt-4 grid gap-x-6 gap-y-2 font-mono text-xs sm:col-start-3 sm:mt-0 sm:grid-cols-2">
                  {category.skills.map(([name, focus]) => (
                    <li
                      key={name}
                      className="group/skill flex items-baseline justify-between gap-3 border-b border-l-[1.5px] border-paper-border/80 border-l-transparent px-2 py-2.5 transition-all duration-300 ease-out hover:border-b-paper-border-dark hover:border-l-accent-orange hover:bg-ink-muted/10 hover:pl-3"
                    >
