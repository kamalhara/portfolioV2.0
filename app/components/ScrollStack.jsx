...
const ScrollStack = ({
  ...
  const getElementOffset = useCallback(
    (element) => {
      if (useWindowScroll) {
        const scroller = scrollerRef.current;
        const rootTop = scroller.getBoundingClientRect().top + window.scrollY;
        let offset = 0;
        let current = element;
  ...
  if (shouldReduceMotion) {
    return (
      <div className={containerClassName} ref={scrollerRef}>
        <div className={`space-y-8 ${innerClassName}`.trim()}>{children}</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div
        className={`scroll-stack-inner min-h-screen px-0 pt-[8vh] pb-[42vh] ${innerClassName}`.trim()}
      >
        {children}
