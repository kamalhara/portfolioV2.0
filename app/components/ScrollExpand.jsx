...
const ScrollExpand = ({
  ...
  useEffect(() => {
    ...
    const readProgress = () => {
      const c = propsRef.current;
      if (!c.enabled || reduceMotion) return 1;
  ...
  const media =
    mediaType === "video" ? (
      ...
      <Image
        ref={mediaRef}
        className={`absolute inset-0 h-full w-full origin-center select-none object-cover [will-change:transform] ${mediaClassName}`.trim()}
        src={src}
        alt={alt}
        fill
        preload={preload}
